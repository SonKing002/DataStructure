...
public class QueueMaze
{
    ...
    static void Main(string[] args)
    {
        ...
        //탐색 >
        //큐을 활용해서 상하좌우 순서로 다음 이동을 할 위치에 대한 탐색
        ...
        //큐 생성
        Queue<Location2D> locationQueue = new Queue<Location2D>();
        //큐의 시작 위치를 추가하고, 탐색 시작
        locationQueue.Enqueue(startLocation);

        while (locationQueue.IsEmpty == false)//탐색
        {
            //현재 위치 반환
            Location2D currentLocation = new Location2D();
            locationQueue.Dequeue(ref currentLocation);

            //맵 및 플레이어 정보 출력
            PrintLocation(maze, currentLocation.row, currentLocation.col, 500);
            ...
            //다음 탐색할 위치를 큐에 추가
            // Todo : 로직 완성 :  탐색할 위치를 큐에 추가
            if (maze.IsValidLocation(currentLocation.row - 1, currentLocation.col))//상
            {
                //움직일 수 있다면 Enqueue 추가
                locationQueue.Enqueue(new Location2D(currentLocation.row - 1, currentLocation.col));
            }
            ...
            {
                locationQueue.Enqueue(new Location2D(currentLocation.row, currentLocation.col - 1));
            }
