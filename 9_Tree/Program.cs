namespace _9_Tree
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //자손노드 추가 (두개가 넘어가면 다중 트리) : 잘 안 만든다. (배열로 트리처럼 뿌려주는 방식으로 구현)
            tree.AddChild("B");
...
#region 이진 트리
/*
 * 자식을 최대 두 개까지 강제하는 경우이다.
 * 좌우 대칭으로 완벽하게 균형이 모두 잘 채워져 있는 트리 = 포화상태
 
 * 상황에 따라서, 한쪽이 쏠리는 현상이 있을 수 있다. = 망함, 불균형 트리
 * 왼쪽, 오른쪽 선택에 따라 방향을 가는 것인데 배열과 같아지는 문제가 있을 수 있음

  count = 0; 널 nullptr 정석  (NULL != 0; 그래도 없는 값을 표현하기 어려움)
 
  - 검색이 빠르다 = 정렬이 되어있어야 한다. 규칙성  ( 왼쪽: 작은 수 -10- 오른쪽 : 큰 수 )
  == 삽입 + 정렬 

 알고리즘 같다 = 같은 성능
 == 배열 정렬 -> 검색을 위한 알고리즘
 사용 이유들이 있어야 한다. -> 인덱스 기반, 포인터 기반, 처음 끝을 알 수 없어서
 */
